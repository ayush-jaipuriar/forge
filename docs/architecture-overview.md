...
## Phase 3 Milestone 6 Cloud Storage Backup Payload Migration

- scheduled backup payload bodies are now stored in Cloud Storage instead of `backupPayloads/{backupId}` Firestore documents, while Firestore remains the operational index for `backups/{backupId}` metadata and `backupOperations/default`
- the payload-pointer and restore-eligibility model now lives in [src/domain/backup/types.ts](/Users/ayushjaipuriar/Documents/GitHub/forge/src/domain/backup/types.ts) and [src/services/backup/backupPayloadStorage.ts](/Users/ayushjaipuriar/Documents/GitHub/forge/src/services/backup/backupPayloadStorage.ts), which matters because backup metadata can now honestly describe where the payload lives and whether a scheduled backup is restorable
- the Functions backup pipeline in [functions/src/backups/pipeline.ts](/Users/ayushjaipuriar/Documents/GitHub/forge/functions/src/backups/pipeline.ts) now uploads payload JSON to deterministic Cloud Storage object paths, persists payload pointers into backup metadata, and cleans up expired payload objects during retention processing
- repo-managed Storage policy now exists in [storage.rules](/Users/ayushjaipuriar/Documents/GitHub/forge/storage.rules) and [firebase.json](/Users/ayushjaipuriar/Documents/GitHub/forge/firebase.json), which means owner-read access for scheduled backup payloads is no longer an undocumented console setting
- the client Firebase seam in [src/lib/firebase/client.ts](/Users/ayushjaipuriar/Documents/GitHub/forge/src/lib/firebase/client.ts) now exposes Firebase Storage, and the retrieval foundation in [src/services/backup/serverBackupRestoreService.ts](/Users/ayushjaipuriar/Documents/GitHub/forge/src/services/backup/serverBackupRestoreService.ts) can resolve scheduled backup metadata into either Cloud Storage or legacy Firestore-backed payload reads
- the Settings workspace in [src/services/settings/settingsWorkspaceService.ts](/Users/ayushjaipuriar/Documents/GitHub/forge/src/services/settings/settingsWorkspaceService.ts) and [src/features/settings/pages/SettingsPage.tsx](/Users/ayushjaipuriar/Documents/GitHub/forge/src/features/settings/pages/SettingsPage.tsx) now surfaces the server-restore foundation honestly by showing restore-ready scheduled backup counts without pretending the full backup-picker UI already exists
- backup payload load failures now report through the monitoring seam, and focused pointer/eligibility coverage exists in [src/tests/domain/backup-payload-storage.spec.ts](/Users/ayushjaipuriar/Documents/GitHub/forge/src/tests/domain/backup-payload-storage.spec.ts); local verification has confirmed root tests, root build, and Functions build after the migration

## Phase 3 Milestone 7 Google Calendar Read Integration

- the old placeholder calendar boundary in [src/services/calendar/calendarIntegrationService.ts](/Users/ayushjaipuriar/Documents/GitHub/forge/src/services/calendar/calendarIntegrationService.ts) is now a real read-side integration service: it owns connect, disconnect, bounded cache refresh, sync-state merging, and collision-summary delivery for Today and Schedule
- Google provider mechanics are isolated in [src/services/calendar/googleCalendarClient.ts](/Users/ayushjaipuriar/Documents/GitHub/forge/src/services/calendar/googleCalendarClient.ts), which matters because the rest of Forge now consumes normalized events and collision summaries instead of raw Google payloads or popup semantics
- local persistence now includes dedicated Calendar stores in [src/data/local/forgeDb.ts](/Users/ayushjaipuriar/Documents/GitHub/forge/src/data/local/forgeDb.ts), [src/data/local/localCalendarStateRepository.ts](/Users/ayushjaipuriar/Documents/GitHub/forge/src/data/local/localCalendarStateRepository.ts), [src/data/local/localExternalCalendarEventRepository.ts](/Users/ayushjaipuriar/Documents/GitHub/forge/src/data/local/localExternalCalendarEventRepository.ts), and [src/data/local/localCalendarSessionRepository.ts](/Users/ayushjaipuriar/Documents/GitHub/forge/src/data/local/localCalendarSessionRepository.ts); this keeps Calendar cache and local access session state separate from user-intent settings and avoids pretending those tokens belong in Firestore
- the local Calendar session is now bound to the authenticated Firebase uid and cleared on sign-out through [src/features/auth/providers/AuthSessionProvider.tsx](/Users/ayushjaipuriar/Documents/GitHub/forge/src/features/auth/providers/AuthSessionProvider.tsx), which closes an important privacy gap for shared-browser or account-switching scenarios
- collision semantics now live in [src/domain/calendar/collisions.ts](/Users/ayushjaipuriar/Documents/GitHub/forge/src/domain/calendar/collisions.ts), where timed Forge blocks are compared against normalized external events to produce `none`, `soft`, or `hard` pressure with explicit constrained windows
- Google event normalization in [src/services/calendar/googleCalendarClient.ts](/Users/ayushjaipuriar/Documents/GitHub/forge/src/services/calendar/googleCalendarClient.ts) now expands multi-day coverage across every constrained date in the local cache instead of indexing only the start date, which prevents later days from silently missing overnight or all-day pressure
- [src/services/routine/routinePersistenceService.ts](/Users/ayushjaipuriar/Documents/GitHub/forge/src/services/routine/routinePersistenceService.ts) now threads Calendar read pressure into both Today and the weekly Schedule workspace, which is the architectural point of the milestone: external events influence decision quality without replacing the routine engine
- [src/features/settings/pages/SettingsPage.tsx](/Users/ayushjaipuriar/Documents/GitHub/forge/src/features/settings/pages/SettingsPage.tsx), [src/features/today/pages/TodayPage.tsx](/Users/ayushjaipuriar/Documents/GitHub/forge/src/features/today/pages/TodayPage.tsx), and [src/features/schedule/pages/SchedulePage.tsx](/Users/ayushjaipuriar/Documents/GitHub/forge/src/features/schedule/pages/SchedulePage.tsx) now surface connection, sync, and collision posture directly, so the user can see what the integration knows instead of treating Calendar pressure as invisible background magic
- the current honesty boundary is important: Forge now supports primary-calendar read access with `calendar.readonly`, bounded local cache refresh, and reconnect-on-expiry behavior, but it does not yet claim long-lived server-managed Calendar tokens

## Phase 3 Milestone 8 Google Calendar Write and Mirror Management

- Calendar mirror policy now lives in [src/domain/calendar/mirrors.ts](/Users/ayushjaipuriar/Documents/GitHub/forge/src/domain/calendar/mirrors.ts), where major-block eligibility, desired mirror shape, and create/update/delete/noop reconciliation are evaluated before any provider call happens; this keeps provider behavior explainable and testable
- Google write-side transport now lives alongside the read adapter in [src/services/calendar/googleCalendarClient.ts](/Users/ayushjaipuriar/Documents/GitHub/forge/src/services/calendar/googleCalendarClient.ts), which matters because the rest of Forge still consumes typed mirror and sync results instead of raw HTTP payloads
- local persistence now includes a dedicated mirror-mapping store in [src/data/local/forgeDb.ts](/Users/ayushjaipuriar/Documents/GitHub/forge/src/data/local/forgeDb.ts) and [src/data/local/localCalendarMirrorRepository.ts](/Users/ayushjaipuriar/Documents/GitHub/forge/src/data/local/localCalendarMirrorRepository.ts), so repeated syncs update the same Google events instead of spraying duplicates
- [src/services/calendar/calendarIntegrationService.ts](/Users/ayushjaipuriar/Documents/GitHub/forge/src/services/calendar/calendarIntegrationService.ts) now owns both scope upgrades to `calendar.events` and explicit mirror reconciliation, which keeps session scope, sync-state truth, and write behavior aligned in one orchestration seam
- the Settings workspace in [src/services/settings/settingsWorkspaceService.ts](/Users/ayushjaipuriar/Documents/GitHub/forge/src/services/settings/settingsWorkspaceService.ts) and [src/features/settings/pages/SettingsPage.tsx](/Users/ayushjaipuriar/Documents/GitHub/forge/src/features/settings/pages/SettingsPage.tsx) now surfaces read status, mirror status, mirror counts, and pending Calendar actions separately, which makes the integration teachable instead of opaque
- Today and Schedule now use shared presentation helpers from [src/domain/calendar/presentation.ts](/Users/ayushjaipuriar/Documents/GitHub/forge/src/domain/calendar/presentation.ts) so stale or error Calendar state is visually louder than raw collision count, and user-facing timestamps are formatted instead of exposing storage-oriented ISO strings
- routine-changing services such as [src/services/routine/dayExecutionService.ts](/Users/ayushjaipuriar/Documents/GitHub/forge/src/services/routine/dayExecutionService.ts), [src/services/settings/dayModeOverrideService.ts](/Users/ayushjaipuriar/Documents/GitHub/forge/src/services/settings/dayModeOverrideService.ts), and [src/services/settings/dayTypeOverrideService.ts](/Users/ayushjaipuriar/Documents/GitHub/forge/src/services/settings/dayTypeOverrideService.ts) now mark Calendar mirrors as `stale` when write mirroring is enabled, which is crucial for preventing false “aligned” states after local routine edits
- collision logic in [src/domain/calendar/collisions.ts](/Users/ayushjaipuriar/Documents/GitHub/forge/src/domain/calendar/collisions.ts) now ignores Forge-managed events when calculating external pressure, preventing the app from reading back its own mirrored events as if they were outside commitments
...
## Phase 4 Milestone 6 Remote Restore and Recovery Completion

- Forge now completes the recovery loop that Phase 3 left intentionally half-open: [src/services/backup/serverBackupRestoreService.ts](/Users/ayushjaipuriar/Documents/GitHub/forge/src/services/backup/serverBackupRestoreService.ts) can already resolve restore-ready scheduled backups, and [src/features/settings/pages/SettingsPage.tsx](/Users/ayushjaipuriar/Documents/GitHub/forge/src/features/settings/pages/SettingsPage.tsx) now turns that into a user-facing staged restore picker instead of only a restore-ready count
- the key architectural decision is that remote recovery does not get its own separate apply path; both local files and scheduled backups now converge into the same `RestoreStage` shape in [src/services/backup/restoreService.ts](/Users/ayushjaipuriar/Documents/GitHub/forge/src/services/backup/restoreService.ts), which keeps schema validation, warning generation, queue clearing, and final apply semantics consistent
- the staged restore surface now carries explicit source metadata, which means the product can honestly say whether the pending restore came from a local file or a scheduled remote backup instead of forcing the user to infer it from the previous button click
- remote restore still stays conservative: only restore-eligible scheduled backups are stageable, ineligible or expired backups are surfaced as non-ready, and apply remains a second explicit confirmation step so compatibility warnings stay visible before local state mutation

## Phase 4 Milestone 7 Platform Service Extraction Inside Firebase Functions

- Forge now has an explicit ownership contract in [src/domain/platform/ownership.ts](/Users/ayushjaipuriar/Documents/GitHub/forge/src/domain/platform/ownership.ts), which is the key architectural move in this milestone: the app can now say which responsibilities belong to the browser, the PWA runtime, or Firebase Functions without confusing shared domain logic with runtime ownership
- the current active Functions-owned companion operations are intentionally narrow and honest: scheduled backups, scheduled notification evaluation, and analytics snapshot regeneration. Those operations are described once in the ownership map and invoked through [src/services/platform/platformFunctionService.ts](/Users/ayushjaipuriar/Documents/GitHub/forge/src/services/platform/platformFunctionService.ts) instead of scattering callable names or backend assumptions around the UI
- [src/services/platform/platformOwnershipService.ts](/Users/ayushjaipuriar/Documents/GitHub/forge/src/services/platform/platformOwnershipService.ts) keeps the ownership map as a reusable workspace rather than a Settings-only constant. This matters because a future admin console, diagnostics page, or native operator surface can consume the same boundary model without reinterpreting platform ownership from docs by hand
- [src/lib/firebase/client.ts](/Users/ayushjaipuriar/Documents/GitHub/forge/src/lib/firebase/client.ts) now exposes a region-bound Functions client helper, and [src/features/settings/hooks/useInvokePlatformOperation.ts](/Users/ayushjaipuriar/Documents/GitHub/forge/src/features/settings/hooks/useInvokePlatformOperation.ts) provides the mutation path for manually invoking the currently supported Functions-backed operations while invalidating the correct downstream workspaces
