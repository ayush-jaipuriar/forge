...
- the current honesty boundary is important: Forge now supports primary-calendar read access with `calendar.readonly`, bounded local cache refresh, and reconnect-on-expiry behavior, but it does not yet claim long-lived server-managed Calendar tokens or Calendar write mirroring
