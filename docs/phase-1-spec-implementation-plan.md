...
- Extended the Milestone 4 mutation path into real day execution: Today can now mark blocks complete or skipped, those changes persist through the local day-instance repository, and queued `upsertDayInstance` writes are coalesced so the latest local day snapshot is what syncs upstream.
