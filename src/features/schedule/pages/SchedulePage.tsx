      ...
      >
        <Stack spacing={2.5} sx={{ minWidth: 0, order: { xs: 2, lg: 1 } }}>
          <SurfaceCard
            ...
            description="Choose a day to inspect the override surface in detail. The board stays high-level so the week is scannable before you start editing posture."
          >
            <Box
              sx={{
                display: 'flex',
          ...
          <SurfaceCard
            ...
            description="These schedule signals stay connected to the analytics layer, but they are translated into planning language instead of dashboard language."
          >
            <Stack spacing={1.25}>
              {globalSignals.length > 0 ? (
                ...
                <Typography variant="body2" color="text.secondary">
                  No cross-week planning signals are elevated right now. The board can stay focused on execution rhythm instead of intervention.
                </Typography>
              )}
            </Stack>
          </SurfaceCard>
        ...
        <Stack spacing={2.5} sx={{ minWidth: 0, order: { xs: 1, lg: 2 } }}>
          ...
          >
            <Stack spacing={2}>
              ...
              <Stack spacing={1}>
                ...
                <Stack direction="row" spacing={1} useFlexGap flexWrap="wrap">
                  {selectedDay.allowedDayTypes.map((option) => (
                    <Button
                      ...
                      disabled={updateDayTypeOverrideMutation.isPending || option.value === selectedDay.dayType}
                      onClick={() =>
                ...
                <Typography variant="body2" color="text.secondary">
                  Overrides are date-scoped reclassifications only. They do not rewrite seeded routine templates.
                </Typography>
              </Stack>
              ...
              <Stack spacing={1}>
                ...
                <Stack direction="row" spacing={1} useFlexGap flexWrap="wrap">
                  <Button
                    ...
                    disabled={updateDayModeMutation.isPending || selectedDay.dayMode === 'lowEnergy'}
                    onClick={() => updateDayModeMutation.mutate({ date: selectedDay.date, dayMode: 'lowEnergy' })}
                  >
                    Low Energy
                  </Button>
                  <Button
                    ...
                    disabled={updateDayModeMutation.isPending || selectedDay.dayMode === 'survival'}
                    onClick={() => updateDayModeMutation.mutate({ date: selectedDay.date, dayMode: 'survival' })}
                  >
                    Survival
                  </Button>
                  <Button
                    ...
                    disabled={updateDayModeMutation.isPending || selectedDay.dayMode === 'normal'}
                    onClick={() => updateDayModeMutation.mutate({ date: selectedDay.date, dayMode: 'normal' })}
                  >
                    Return to Normal
                  </Button>
                </Stack>
          ...
          <SurfaceCard
            ...
            description="Calendar pressure is visible here as a planning constraint. Forge still owns the routine model and all override behavior."
            action={
              <Chip
